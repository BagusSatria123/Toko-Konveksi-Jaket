...
<?php 
    ...
    if($group=='-' || $group!=$d['bulan']){
        $bulan=$d['bulan'];
        $query=$this->db->query("SELECT jual_nofak,DATE_FORMAT(jual_tanggal,'%M %Y') AS bulan,DATE_FORMAT(jual_tanggal,'%d %M %Y') AS jual_tanggal,d_jual_produk_id,d_jual_produk_nama,d_jual_produk_warna,d_jual_produk_harga,d_jual_qty,d_jual_diskon,SUM(d_jual_total) as total FROM t_jualproduk JOIN t_jualproduk_detail ON jual_nofak=d_jual_nofak WHERE DATE_FORMAT(jual_tanggal,'%M %Y')='$bulan'");
        $t=$query->row_array();
        $tots=$t['total'];
        if($group!='-')
        echo "</table><br>";
...
$nomor=1;
    ...
    $group=$d['bulan'];
        ...
        <tr>
                ...
                <td style="vertical-align:top;padding-left:5px;"><?php echo $d['d_jual_produk_warna']; ?></td>
                <td style="vertical-align:top;padding-left:5px;text-align:right;"><?php echo 'Rp '.number_format($d['d_jual_produk_harga']); ?></td>
